...
def apply_keyframes(transform, blur, keys):
    """Set per-frame keyframes for Center, Size, Blend (and Blur softness)."""
    for k in keys:
        f = k["f"]
        # Center is a Point input; assign [x, y] at the comp-local frame.
...
def import_words(timeline, data):
    ...
    for word in words:
        # Position the playhead at the word's start, then drop a Text+ title.
        timeline.SetCurrentTimecode(frames_to_timecode(word["startFrame"], fps))
        ...
        apply_keyframes(transform, blur, word["keys"])
        inserted += 1
